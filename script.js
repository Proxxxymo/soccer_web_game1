...
function drawBackground() {
    ctx.fillStyle = '#C0C0C0'; // Легкий серый цвет для всего фона
    ctx.fillRect(0, 0, canvas.width, canvas.height);
}

function drawGameField() {
    // Рисуем зеленый прямоугольник на всем игровом поле
    ctx.fillStyle = 'green';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
}
...
function drawGoals() {
    ctx.lineWidth = 5;
    for (let row = 0; row < 3; row++) {
        // Рисуем зеленый фон для ячеек ворот
        ctx.fillStyle = 'green';
        ctx.fillRect(0, row * cellHeight, cellWidth, cellHeight);
        ctx.fillRect(6 * cellWidth, row * cellHeight, cellWidth, cellHeight);
        // Рисуем рамку ворот
